...
public class ReflectionMain {
	...
	public static void main(String[] args) {
		...
		// Write to schema & data to avro file
		try {
			...
			DataFileWriter<ReflectedCustomer> out = new DataFileWriter<>(writer).setCodec(CodecFactory.deflateCodec(9))
					.create(schema, file);
			out.append(
					new ReflectedCustomer("Bill", "Clark", "The Rocket", new Address("Test addres", null, "TesCity")));
			out.close();
		} catch (IOException e) {
			e.printStackTrace();
		}
		...
		try {
			...
			for (ReflectedCustomer reflectedCustomer : in) {
				...
				System.out.println(reflectedCustomer.getPrimaryAddress());
			}
